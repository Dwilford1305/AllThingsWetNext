...
async function uploadBusinessLogo(request: AuthenticatedRequest) {
  try {
    await connectDB()
...
export const POST = withAuth(uploadBusinessLogo)
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    let business
    if (user.role === 'super_admin') {
      // Super admin can delete logos from any business
      business = await Business.findOne({ id: businessId })
    } else {
      // Regular users can only access businesses they own
      business = await Business.findOne({ 
