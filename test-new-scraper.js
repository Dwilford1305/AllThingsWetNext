...
class WetaskiwinBusinessScraper {
  constructor() {
  ...
  parseNameAndContact(nameContactText) {
    if (!nameContactText) {
  ...
  looksLikePersonName(word1, word2) {
...
async function testNewScraper() {
  ...
  try {
    ...
    if (businesses.length > 0) {
      console.log(`\n=== FIRST 10 BUSINESSES ===`);
      businesses.slice(0, 10).forEach((business, i) => {
        console.log(`\n${i + 1}. ${business.name}`);
        console.log(`   Contact: ${business.contact || 'N/A'}`);
        console.log(`   Address: ${business.address}`);
        console.log(`   Phone: ${business.phone || 'N/A'}`);
        console.log(`   Category: ${scraper.categorizeBusinessType(business.name)}`);
      });
      
      // Show some statistics
      const categories = {};
      businesses.forEach(business => {
        const category = scraper.categorizeBusinessType(business.name);
